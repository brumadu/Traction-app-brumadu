import { Typography, Image, Col, Row, Card, Space, Button, Popover, Modal } from "antd";
...
export function AssetCard(assets: assetsProps) {
  ...
  return (
  <>
    <Card hoverable={true} style={{ width: '100%', marginBottom: 10 }} onClick={showModal}>
      <Row>
        <Col span={14}>
          ...
          <Col>
            <Title level={5}>{'Healthscore: ' + assets.data.healthscore + "%"}</Title>
          </Col>
          <Col >
        ...
        <Col span={10}>
          <Image preview={false} src={assets.data.image} style={{ borderRadius: 10, maxHeight: '20vh', width: '17vh' }} />
        </Col>
      </Row>
    </Card>
    <Modal centered open={isModalOpen} footer={null} onCancel={handleCancel} width={"50%"}>
      <Title level={3}> {assets.data.name} </Title>
      <Row>
        <Col span={8}>
          <Image src={assets.data.image} style={{ borderRadius: 10, width: 250 }} />
          {
